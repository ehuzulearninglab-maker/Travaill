...
Ajouter dans Vercel, section **Settings > Environment Variables** :
...
`GEMINI_MODEL` est facultatif. Si cette variable est absente, l'application utilise `gemini-2.5-flash-lite`.

## Fonctionnement

1. Le GPT personnalisé envoie la fiche à `/api/fiches/import`.
2. L'application fait une première normalisation locale.
3. Si `GEMINI_API_KEY` existe, Gemini reformate la fiche en JSON structuré.
