      ...
      <section className="rounded-md border border-stone-200 bg-white p-5 shadow-doux">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-stone-600">Aperçu rapide des traitements reçus par l’application.</p>
          </div>
          <Link href="/admin/gemini" className="bouton-secondaire">
            Voir le suivi
          </Link>
            ...
            <tbody className="divide-y divide-stone-100">
              {activities.map((activity) => (
                <tr key={activity.id}>
                  ...
                  <td className="px-4 py-3 text-stone-600">{activity.statut}</td>
                  <td className="px-4 py-3 text-stone-600">{activity.tokens_total}</td>
                </tr>
              ))}
              {activities.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-stone-600">
